...
dec_jan_feb = df.loc[df["date"].dt.month.isin([5, 6, 7, 8, 9])]
...
chart = (
    ...
    .encode(
        ...
        y=alt.Y("value:Q", axis=alt.Axis(title="Precipitation mean")),
        color="Bundesland:N",
        shape="Bundesland:N",
    )
    .properties(
        width=600,
        height=400,
        title="Precipitation summer months from 1993 to 2022",
    )
    .interactive()
)
